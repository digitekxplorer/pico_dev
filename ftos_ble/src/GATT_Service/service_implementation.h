...
static int custom_service_write_callback(hci_con_handle_t con_handle, uint16_t attribute_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size){
	...
	if (attribute_handle == service_object.characteristic_b_handle) {
		...
		printf("Charactertic_b command: %s.\n", service_object.characteristic_b_value);
		// clear ble_input buffer
		memset(ble_input, 0, sizeof(ble_input)) ;  // fill a block of memory with a specific value
		memcpy(ble_input, buffer, buffer_size) ;   // copy to buffer in defs.h
		printf("Text command from client: %s.\n", ble_input);
		    
        // **************
        // Call to FreeRTOS task!!!!
        // **************
        // Send a notification to the handler task; code in ftos_tasks.h
        xTaskNotifyGive( xBLEinput_HandlerTask);      // Call to FreeRTOS task!!!!
...
void custom_service_server_init(char * a_ptr, char * b_ptr, char * c_ptr){
